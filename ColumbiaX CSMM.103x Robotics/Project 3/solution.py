...
class ForwardKinematics(object):
    ...
    def compute_transforms(self, link_names, joints, joint_values):
        ...
        for i in range(len(joints)):
            joint = joints[i]
            # Find the infomation of the joint
            try:
            ...
            D = tf.transformations.translation_matrix(joint.origin.xyz)
            ...
            if joint.type == 'revolute':
             	R_J = tf.transformations.rotation_matrix(joint_value, joint.axis)
            else:
             	R_J = tf.transformations.identity_matrix()
